...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build curated Stage1~3 files from seed text, based on nemo_dream_step1."
    )
    parser.add_argument("--input", default="data/curated/sample_input_data.jsonl")
...
def load_rows(path: Path, limit: int | None) -> list[dict[str, Any]]:
    ...
    for idx, line in enumerate(path.read_text(encoding="utf-8").splitlines()):
        ...
        rows.append({"id": str(raw.get("id") or f"seed-{idx:06d}"), "text": str(text), "metadata": raw.get("metadata") or {}})
...
def nim_decompose(text: str, args: argparse.Namespace) -> dict[str, Any]:
    resp = client(args.base_url).chat.completions.create(
        model=args.model,
        messages=[
    ...
    return normalize_decomposition(payload, text, args.cultural_map)


def nim_map_refs(row: dict[str, Any], args: argparse.Namespace) -> list[dict[str, Any]]:
    refs = row["decomposed"].get("cultural_refs", [])
    if not refs:
        return []
    candidates = []
    for ref in refs:
        hit = cultural_index(args.cultural_map).get(normalize_term(ref["term"]))
        if hit:
...
def nim_rewrite(row: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    prompt = {
        "source_text": row["source_text"],
        "decomposed": row["decomposed"],
...
def main() -> None:
    args = parse_args()
    input_rows = load_rows(Path(args.input), args.limit)
    out_dir = Path(args.out_dir)
    print(f"[curated] loaded {len(input_rows)} rows from {args.input}")

    stage1 = []
    for row in input_rows:
        decomposed = nim_decompose(row["text"], args)
        stage1.append({"id": row["id"], "text": row["text"], "metadata": row["metadata"], "decomposed": decomposed})
    write_jsonl(stage1, out_dir / "stage1.jsonl")
    print(f"[curated] wrote {out_dir / 'stage1.jsonl'}")
    ...
    for row in stage1:
        ...
        stage2[-1]["mapped_refs"] = nim_map_refs(stage2[-1], args)
    write_jsonl(stage2, out_dir / "stage2.jsonl")
    print(f"[curated] wrote {out_dir / 'stage2.jsonl'}")

    stage3 = []
    for row in stage2:
        generation = nim_rewrite(row, args)
        stage3.append({**row, "generation": generation})
    write_jsonl(stage3, out_dir / "stage3.jsonl")
    print(f"[curated] wrote {out_dir / 'stage3.jsonl'}")
